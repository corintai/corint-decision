...
打开 <http://127.0.0.1:5173>。默认读取项目根目录 `repository/` 下的 YAML / YML / JSON 文件，按实际相对路径显示可折叠目录树；搜索文件、目录或资源名称时会展开匹配路径。每次启动和点击校验时都会检查磁盘内容版本：版本未变时恢复并校验当前浏览器草稿；磁盘更新或旧草稿没有版本信息时，先备份旧草稿，再加载最新文件。页面提供「导出旧草稿」，可将备份导出后重新导入；备份保留最近一次被替换的工作区。「重新加载 repository」也可手动从磁盘更新，当前页面内支持撤销。原示例工作区的缓存单独保留，不会覆盖新工作区。读取目录不跟随符号链接。
