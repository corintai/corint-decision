...
1. **统一可观测性框架** - 使用OpenTelemetry SDK作为统一instrumentation层
2. **Prometheus metrics导出** - 通过/metrics endpoint暴露指标
3. **分布式追踪支持** - 支持OTLP协议，可导出到Jaeger/Zipkin
4. **corint-server集成** - Server自动暴露/metrics endpoint
5. **示例和文档** - 提供完整的使用示例和runtime README文档

**实现细节:**
- ✅ `crates/corint-runtime/src/observability/otel.rs` - OpenTelemetry初始化模块
...
**功能特性:**
- Counter、Histogram、UpDownCounter等指标类型
- 自动metrics导出（默认60秒间隔）
- 支持自定义labels和attributes
- 可配置OTLP endpoint用于traces导出
- 与现有tracing系统共存

**影响:**
- 生产就绪度提升：支持标准可观测性工具链
- 监控能力增强：可集成Prometheus、Grafana等
- 调试能力提升：支持分布式追踪
- 零性能影响：metrics为冷路径，仅在scrape时计算
...
**完成项:**
- ✅ 添加OpenTelemetry可观测性支持（统一实现）
  - ✅ 使用OpenTelemetry SDK作为统一instrumentation层
...
| 任务 | 文件/范围 | 估计工作量 | 来源 |
|------|---------|-----------|------|
| ~~修复server中的panic!()~~ | ~~rest.rs~~ | ~~1天~~ | ✅ 已完成 |
| ~~修复extractor中的unwrap()~~ | ~~extractor.rs~~ | ~~2天~~ | ✅ 已完成 |
...
| 任务 | 范围 | 估计工作量 | 来源 | 状态 |
|------|------|-----------|------|------|
| ~~清理死代码和警告~~ | ~~全项目~~ | ~~1天~~ | 优化建议 | ✅ 已完成 |
| ~~统一错误处理~~ | ~~全项目~~ | ~~3天~~ | 优化建议 | ✅ 已完成 |
| 标准化公共API | 全项目 | 2天 | 优化建议 | |
| 优化克隆操作 | repository, runtime | 2天 | 优化建议 | |
| 提升SDK测试覆盖 | corint-sdk | 2天 | 优化建议 | |
| 实现数据源客户端 | runtime | 3天 | 优化建议 | |

#### P1.2 规则DSL功能增强

| 任务 | 范围 | 估计工作量 | 来源 | 状态 |
|------|------|-----------|------|------|
| **[新增] 动态阈值实现** | **dynamic_threshold功能** | **5天** | **规则改进** | |
| **[新增] 规则依赖和优先级** | **depends_on, priority, groups** | **3天** | **规则改进** | |
| **[新增] 并行执行步骤** | **parallel with merge** | **3天** | **规则改进** | |
| **[新增] Pipeline高级特性测试** | **parallel_execution_test.yaml** | **2天** | **规则改进** | |
| **[新增] 规则高级特性测试** | **rule_advanced_tests.rs** | **2天** | **规则改进** | |

#### P1.3 外部API集成

| 任务 | 范围 | 估计工作量 | 来源 | 状态 |
|------|------|-----------|------|------|
| **[新增] Chainalysis API集成** | **configs/apis/chainalysis.yaml** | **2天** | **规则改进** | |
| **[新增] FingerprintJS API集成** | **configs/apis/fingerprintjs.yaml** | **2天** | **规则改进** | |
| **[新增] MaxMind GeoIP集成** | **configs/apis/maxmind.yaml** | **2天** | **规则改进** | |

**总计: ~30天** (原13天，新增17天 - 已完成项已划线)
...
|------|------|-----------|------|------|
| 性能基准测试 | 全项目 | 2天 | 优化建议 | |
| ~~完善metrics框架~~ | ~~runtime~~ | ~~2天~~ | 优化建议 | ✅ 已完成(OTel) |
| 字符串分配优化 | 全项目 | 1天 | 优化建议 | |
...
| 任务 | 范围 | 估计工作量 | 来源 | 状态 |
|------|------|-----------|------|------|
| **[新增] 创建登录风控规则集** | **login_risk.yaml** | **1天** | **规则改进** | ⚠️ 部分完成 |
| **[新增] 添加impossible_travel规则** | **impossible_travel.yaml** | **1天** | **规则改进** | ✅ 已完成 |
| **[新增] 添加off_hours活动检测** | **off_hours_activity.yaml** | **1天** | **规则改进** | |
| **[新增] 添加密码变更风险检测** | **password_change_risk.yaml** | **1天** | **规则改进** | ✅ 已完成 |
| **[新增] 设备模拟器检测** | **device_emulator.yaml** | **1天** | **规则改进** | ✅ 已完成 |
| **[新增] 设备欺骗检测** | **device_spoofing.yaml** | **1天** | **规则改进** | ✅ 已完成 |
| **[新增] 创建KYC验证规则集** | **kyc_verification.yaml** | **2天** | **规则改进** | |
| **[新增] 身份重用检测** | **identity_reuse.yaml** | **1天** | **规则改进** | |
| **[新增] 收入不一致检测** | **income_inconsistency.yaml** | **1天** | **规则改进** | |

#### P2.4 特征工程增强

| 任务 | 范围 | 估计工作量 | 来源 | 状态 |
|------|------|-----------|------|------|
| **[新增] 统计异常检测特性** | **zscore, percentile_check** | **3天** | **规则改进** | |
| **[新增] 关联分析特性** | **IP/设备/用户关联** | **2天** | **规则改进** | |
| **[新增] 外部服务集成测试** | **API mock测试** | **2天** | **规则改进** | |
| **[新增] 错误处理测试** | **error_handling_tests.rs** | **2天** | **规则改进** | |

**总计: ~28天** (原10天，新增18天 - 已完成项已划线)
...
| 任务 | 范围 | 估计工作量 | 来源 | 状态 |
|------|------|-----------|------|------|
| **[新增] 高风险钱包检测** | **high_risk_wallet.yaml** | **2天** | **规则改进** | |
| **[新增] 洗交易模式检测** | **wash_trading.yaml** | **2天** | **规则改进** | |
| **[新增] 可观测性测试** | **observability_tests.rs** | **1天** | **规则改进** | |
...
#### Future.1 LLM集成 (暂缓 - 待功能稳定)

| 任务 | 范围 | 估计工作量 | 来源 | 状态 |
|------|------|-----------|------|------|
| **[新增] LLM推理步骤** | **管道LLM分析** | **5天** | **规则改进** | *暂缓* |
| **[新增] infer动作** | **异步AI分析** | **3天** | **规则改进** | *暂缓* |
| **[新增] LLM集成测试** | **llm_integration_tests.rs** | **2天** | **规则改进** | *暂缓* |

#### Future.2 SDK和客户端

| 功能 | 优先级 | 依赖 | 来源 |
|------|--------|------|------|
| Python SDK | 高 | 稳定API | 优化建议 |
| TypeScript/JavaScript SDK | 高 | 稳定API | 规则改进 |
| Go SDK | 中 | 稳定API | 规则改进 |
| Java SDK | 中 | 稳定API | 规则改进 |

#### Future.3 管理界面

| 功能 | 优先级 | 依赖 | 来源 |
|------|--------|------|------|
| Web UI | 高 | REST API完善 | 优化建议 |
| 规则版本控制和回滚 | 高 | Repository扩展 | 规则改进 |
| A/B测试框架 | 中 | Web UI | 规则改进 |
| 规则性能分析仪表板 | 中 | Observability | 规则改进 |

#### Future.4 部署和运维

| 功能 | 优先级 | 依赖 | 来源 |
|------|--------|------|------|
| Kubernetes部署 | 中 | Docker模板 | 优化建议 |
| Docker部署模板 | 高 | - | 规则改进 |
| Terraform模块 | 中 | K8s | 规则改进 |
| CI/CD流水线模板 | 中 | - | 规则改进 |
...
| 功能 | 优先级 | 依赖 | 来源 |
|------|--------|------|------|
| GraphQL API | 中 | 核心稳定 | 优化建议 |
| gRPC API | 中 | 核心稳定 | 规则改进 |
| WebSocket支持 | 低 | 核心稳定 | 规则改进 |
| 机器学习模型集成 | 低 | 性能优化 | 优化建议 |
| 预构建规则库 | 中 | 规则稳定 | 规则改进 |
| 合规模板 (PCI-DSS, GDPR) | 中 | 规则稳定 | 规则改进 |
...
**功能描述:**
实现自适应阈值，使用统计方法（百分位数、标准差、移动平均）动态调整规则阈值。
...
**TODO:**
- [ ] 在AST中添加DynamicThreshold节点
- [ ] 实现percentile、stddev、moving_average方法
- [ ] 添加缓存机制避免重复计算
- [ ] 支持时间分段（工作时间/非工作时间不同阈值）
- [ ] 添加测试用例
...
**TODO:**
- [ ] 在Rule AST中添加priority、depends_on、group字段
- [ ] 实现规则依赖解析和验证（检测循环依赖）
- [ ] 实现规则执行顺序排序（拓扑排序）
- [ ] 实现互斥组逻辑（同组只执行最高优先级匹配的规则）
- [ ] 添加依赖规则结果访问（context.rules.xxx）
- [ ] 添加测试用例
...
**功能描述:**
在Pipeline中支持并行执行多个步骤，并提供不同的合并策略。
...
**TODO:**
- [ ] 在Pipeline AST中添加ParallelStep节点
- [ ] 实现并行执行器（使用tokio::spawn）
- [ ] 实现merge策略（all/any/first）
- [ ] 添加超时处理
- [ ] 处理部分失败场景
- [ ] 添加并发限制配置
- [ ] 添加测试用例
...
#### 20.1 Chainalysis - 区块链风险分析
- 钱包地址风险评分
- 制裁列表检查
- 暗网/混币器检测

#### 20.2 FingerprintJS - 设备指纹
- 设备唯一性识别
- 机器人检测
- 隐身模式检测

#### 20.3 MaxMind GeoIP2 - IP情报
- 精确地理定位
- VPN/代理检测
- Tor出口节点检测
- 匿名服务检测
...
**TODO:**
- [ ] 创建 `configs/apis/chainalysis.yaml`
- [ ] 创建 `configs/apis/fingerprintjs.yaml`
- [ ] 创建 `configs/apis/maxmind.yaml`
- [ ] 实现API客户端和响应解析
- [ ] 添加速率限制和缓存
- [ ] 添加错误处理和fallback
- [ ] 添加集成测试
...
**已完成:**
- ✅ Impossible Travel Detection ([impossible_travel_detection.yaml](repository/library/rules/fraud/impossible_travel_detection.yaml))
- ✅ Password Change Risk ([password_change_risk.yaml](repository/library/rules/fraud/password_change_risk.yaml))
...
**TODO:**
- [ ] 创建 `library/rules/account/off_hours_activity.yaml`
- [ ] 实现hour()时间函数
- [ ] 支持工作时间配置（可按用户时区调整）
- [ ] 添加到login_risk规则集
...
**已完成:**
- ✅ Device Emulator Detection ([device_emulator.yaml](repository/library/rules/fraud/device_emulator.yaml))
- ✅ Device Spoofing Detection ([device_spoofing.yaml](repository/library/rules/fraud/device_spoofing.yaml))
...
**TODO:**
- [ ] 创建 `library/rules/credit/identity_reuse.yaml`
- [ ] 创建 `library/rules/credit/income_inconsistency.yaml`
- [ ] 创建 `library/rulesets/kyc_verification.yaml`
- [ ] 添加测试用例
...
**TODO:**
- [ ] 创建 `library/rules/crypto/high_risk_wallet.yaml`
- [ ] 创建 `library/rules/crypto/wash_trading.yaml`
- [ ] 依赖: 先完成Chainalysis API集成
- [ ] 添加测试用例
...
**TODO:**
- [ ] 在FeatureOperator枚举中添加新操作符
- [ ] 实现zscore计算逻辑
- [ ] 实现percentile_check逻辑
- [ ] 实现velocity_ratio逻辑
- [ ] 添加测试用例
- [ ] 更新文档
...
## 测试覆盖补充 (来自RULE_IMPROVEMENT_RECOMMENDATIONS.md)

### 27. 🔴 [P0] 特征工程测试套件

**严重性:** Critical - 核心功能严重缺失测试

**当前状态:** 覆盖率仅5%

**TODO:**
- [ ] 创建 `crates/corint-runtime/tests/feature_engineering_tests.rs`
- [ ] 测试features.xxx命名空间访问
- [ ] 测试count_distinct操作符
- [ ] 测试percentile计算
- [ ] 测试velocity计算
- [ ] 测试聚合函数（sum, avg, max）
...
**TODO:**
- [ ] 创建 `crates/corint-runtime/tests/expression_tests.rs`
- [ ] 测试逻辑运算符（AND, OR, NOT）
- [ ] 测试比较运算符（==, !=, >, <, >=, <=）
- [ ] 测试成员运算符（in, contains）
- [ ] 测试时间函数（hours_since, days_since）
- [ ] 测试字符串函数
- [ ] 测试数学运算
...
**TODO:**
- [ ] 创建 `crates/corint-runtime/tests/context_tests.rs`
- [ ] 测试event.*命名空间访问
- [ ] 测试context.*命名空间访问
- [ ] 测试vars.*命名空间访问
- [ ] 测试跨步骤数据流
...
### 30. 🟡 [P1] Pipeline高级特性测试

**TODO:**
- [ ] 创建 `repository/tests/pipelines/parallel_execution_test.yaml`
- [ ] 测试并行执行with merge strategies
- [ ] 测试分支执行（branch）
- [ ] 测试聚合步骤（aggregate）
- [ ] 测试超时处理
- [ ] 测试部分失败场景
...
**TODO:**
- [ ] 创建 `crates/corint-runtime/tests/rule_advanced_tests.rs`
- [ ] 测试负分数规则
- [ ] 测试动态阈值
- [ ] 测试规则依赖
- [ ] 测试规则优先级排序
...
**TODO:**
- [ ] 创建 `crates/corint-runtime/tests/error_handling_tests.rs`
- [ ] 测试fallback策略
- [ ] 测试retry逻辑
- [ ] 测试circuit breaker
- [ ] 测试错误链传播
...
**TODO:**
- [ ] 创建 `crates/corint-runtime/tests/observability_tests.rs`
- [ ] 测试metrics采集
- [ ] 测试日志输出
- [ ] 测试trace propagation
- [ ] 测试采样率配置
