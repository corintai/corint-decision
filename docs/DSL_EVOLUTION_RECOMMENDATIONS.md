...
| `experimental` | 有部分实现，但边界或行为验收未完成 | 显式启用；严格 Core 不接受，不能作为生产支持示例 |
| `planned` | 设计提案或尚未实现 | 明确标注，严格 Core 拒绝 |
| `deprecated` | 曾受支持、仍有兼容测试的旧写法 | 给出替代写法、诊断和移除版本 |

本提案中的 Core 清单处于“候选、待验收”阶段。现有 `docs/cdl/` 中的“✅ Implemented”需要逐项核验；仅能解析或编译，不足以升级为 `supported`。
...
生成可执行策略需要以下三类版本化信息。后两类是包/控制面的公共契约，不是新增的 CDL 顶层语法；具体 schema 尚待实现。
...
下列要求同样尚未落地。阶段 0 验证公共 schema、接口约束、独立工具链和最小往返；涉及真实 Work 客户端、在线资源或生产反馈的用例，在对应集成阶段完成后才能声明产品能力。契约 mock 的通过不等于真实产品集成通过。
...
现有 [CI](../.github/workflows/ci.yml) 已执行 workspace tests，但这不等于已覆盖文档中的全部示例。新增 conformance runner 应接入该测试链路，并补充 schema、文档 fixture 映射、生成器模板和跨产品契约的一致性检查。产品集成测试与公共契约测试分别报告，不通过移除某个适配入口来隐藏语义差异。
