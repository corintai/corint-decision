...
可选的操作员 `feature_pipeline` 配置将[特征输入准备](feature-pipeline.md)接入同一 HTTP 决策入口；计算内核仍只消费校验后的输入。

共享多租户部署使用独立的 `CORINT_TENANT_CONFIG` 入口，见[多租户决策宿主](multi-tenancy.md)。本页说明原有单实例模式；单租户 Journal 默认租户名为 `local`。
...
- 该单实例模式不提供企业身份、TLS、多租户路由或跨节点发布；已配置业务证据时，新请求重读本地信任清单消费撤销；该本地模式不应通过公网代理暴露。
