...
由 `CORINT_CORE_CONFIG` 显式启用，当前配置版本为 **`2`**。
服务启动和重载均读取同一文件 repository；内存引擎只是已验收 repo 版本的派生快照。
...
- 不提供企业身份、TLS、多租户、在线吊销或跨节点发布；该本地模式不应通过公网代理暴露。
- 文件 repository 当前使用 Unix 的受限文件读取。数据库/API repository 和旧 REST/gRPC/FFI
  未自动获得严格 Core 保证；本次没有迁移既有客户策略的语义。
