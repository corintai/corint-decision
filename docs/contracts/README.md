...
[多租户决策宿主](multi-tenancy.md)定义共享 HTTP 入口的租户身份、委托权限、资源隔离、可靠幂等和部署生命周期。原本地单租户入口默认租户名为 `local`。
...
需要部署约束的调用方必须额外运行本检查。[实验性严格 Core 服务端](core-server.md) 现已在本地操作者信任域内接入共享检查、固定样例重跑、批准指纹列表和 repo 原子重载。配置 v2 以文件 repo 为唯一策略来源，启动与重载共用验收，发布历史归 repo 管理；完整生产授权、可信远端证明与跨节点发布仍待完成。
