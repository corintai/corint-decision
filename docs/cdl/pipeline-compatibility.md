...
## Pipeline Compatibility Reference (v0.1)
...
**Compatibility description (unverified):**
- **Rules** detect and score individual risk patterns (Unverified compatibility behavior)
- **Rulesets** produce **signals** (`approve`, `decline`, `review`, `hold`, `pass`) based on rule results (Unverified compatibility behavior)
- **Pipelines** execute rulesets and route events through processing steps (Unverified compatibility behavior)
- **Pipeline-level decision logic** can map ruleset signals to final results (Unverified compatibility behavior)
...
- `next: end` - Explicitly ends step execution and proceeds to `decision`
...
- Strict Core requires explicit `next` on non-router steps; omitted successors in legacy syntax have no Core guarantee
- `decision` section is the terminal phase for making final actions

### 1.3 When Condition (Unverified compatibility behavior)
...
### 2.1 Legacy step types (see gaps below)
...
### 2.3 Historical Pipeline Illustration (unverified; includes unsupported API fields)
...
## 4. Decision Logic (Unverified compatibility behavior)
...
### 8.1 Legacy fields and known gaps

This historical syntax describes the following fields; use [the Core Pipeline reference](pipeline.md) for the tested subset:
...
- **Layer 1 - Rules** (unverified) - Detect and score individual risk patterns
- **Layer 2 - Rulesets** (unverified) - Evaluate rules and produce **signals** via `conclusion` blocks
  - Signals: `approve`, `decline`, `review`, `hold`, `pass`
  - Actions: `KYC`, `OTP`, `2FA`, `BLOCK_DEVICE`, etc.
  - A ruleset can contain one or more rules
- **Layer 3 - Pipelines** (unverified) - Orchestrate rulesets, route events, make final decisions
...
- Modular composition (no measured code-reduction claim)
...
| Date | Changes |
|---|---|
| 2026-09-05 | 保留历史兼容参考，明确尚未验收的行为和已知实现缺口。 |
