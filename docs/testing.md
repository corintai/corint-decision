...
nested short-circuit paths and call-result scoping. For Pipelines, also exercise
selected and unselected branches, guards and child-call score isolation. Errors
must remain errors rather than being treated as unmatched Rules or successful decisions.

The `condition_trace_short_circuit` and `result_dependent_router` cases in the
[conformance manifest](../tests/conformance/cdl_core/manifest.yaml) cover nested
conditions, skipped operands, local scores and actual calls. Run the
...
The [test-suite JSON Schema](contracts/schema/test-suite.json) is embedded in the shared toolchain and
...
validation and behavior testing establish evidence for the declared local examples.
Cross-product interoperability requires independent acceptance.

## Complete CDL payment review project

The [payment review project](../CDL/examples/payment-review/guide.md) follows one authored
flow through Feature calculations, List predicates, Service calls, explicit host input mapping
and imported Core decisions. Its tests read the published source files directly. Run:

```bash
cargo test -p corint-decision-engine --features sqlx,redis --test cdl_payment_review --locked
```

The harness creates a disposable SQLite database, local HTTP server and controlled Redis
protocol server. It does not contact the example provider URL or require a live database,
