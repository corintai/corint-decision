...
# External POSTGRES_URL/CLICKHOUSE_URL/REDIS_URL/RISINGWAVE_URL must point to test databases.
set -eo pipefail
...
case "${1:-}" in
    --sqlite|--postgres|--clickhouse|--redis|--risingwave|--all) DATASOURCE=${1#--} ;;
    "")
        if [ -t 0 ]; then
            read -r -p 'Datasource (sqlite/postgres/clickhouse/redis/risingwave/all) [sqlite]: ' DATASOURCE
            DATASOURCE=${DATASOURCE:-sqlite}
        else DATASOURCE=sqlite; fi ;;
    *) echo "Usage: $0 [--sqlite|--postgres|--clickhouse|--redis|--risingwave|--all]" >&2; exit 2 ;;
esac
case "$DATASOURCE" in sqlite|postgres|clickhouse|redis|risingwave|all) ;; *) exit 2 ;; esac
...
STARTUP_TIMEOUT=${CORINT_E2E_STARTUP_TIMEOUT_SECS:-60}
[[ "$STARTUP_TIMEOUT" =~ ^[1-9][0-9]*$ ]] || { echo 'Startup timeout must be positive seconds' >&2; exit 2; }
log_info() { echo "[INFO] $*"; }
...
if [ "$DATASOURCE" = all ]; then
    ...
    failed=0
    for ds in sqlite postgres clickhouse redis risingwave; do
        reason=""
        case "$ds" in
            ...
            risingwave) [ -n "${RISINGWAVE_URL:-}" ] || reason='RISINGWAVE_URL not configured' ;;
        esac
        if [ -n "$reason" ]; then
            status=skipped
...
cleanup() {
    ...
    if [ "$RISINGWAVE_SCHEMA_CREATED" = true ]; then
        if ! PGCONNECT_TIMEOUT=5 psql -X -q -v ON_ERROR_STOP=1 -v rw_schema="$RISINGWAVE_SCHEMA" "$RISINGWAVE_URL" <<'SQL_CLEANUP'
DROP SCHEMA :"rw_schema" CASCADE;
SQL_CLEANUP
        then
            log_error "Failed to remove test schema $RISINGWAVE_SCHEMA"
            rm -rf "$RUN_DIR"
            return 1
        fi
    fi
...
case "$DATASOURCE" in
    ...
    risingwave)
        : "${RISINGWAVE_URL:?Set RISINGWAVE_URL to a RisingWave test database}"
        command -v psql >/dev/null
        version=$(PGCONNECT_TIMEOUT=5 psql -X -A -t -v ON_ERROR_STOP=1 "$RISINGWAVE_URL" -c 'SELECT version()')
        case "$version" in *RisingWave*) ;; *) log_error 'RISINGWAVE_URL must point to real RisingWave'; exit 2 ;; esac
        RISINGWAVE_SCHEMA="corint_e2e_$(python3 -c 'import uuid; print(uuid.uuid4().hex)')"
        export RISINGWAVE_SCHEMA
        PGCONNECT_TIMEOUT=5 psql -X -q -v ON_ERROR_STOP=1 -v rw_schema="$RISINGWAVE_SCHEMA" "$RISINGWAVE_URL" <<'SQL_CREATE'
CREATE SCHEMA :"rw_schema";
SQL_CREATE
        RISINGWAVE_SCHEMA_CREATED=true
        PGCONNECT_TIMEOUT=5 psql -X -q -v ON_ERROR_STOP=1 -v rw_schema="$RISINGWAVE_SCHEMA" "$RISINGWAVE_URL" -f tests/scripts/risingwave_fixture.sql ;;
    redis)
...
if kind == 'risingwave':
    shutil.copytree(repo / 'templates/risingwave', repo, dirs_exist_ok=True)
else:
...
if kind == 'risingwave':
    source = {'type': 'feature_store', 'provider': 'risingwave', 'connection_string': os.environ['RISINGWAVE_URL'],
              'options': {'max_connections': '3', 'connection_timeout': '2', 'query_cache_ttl_secs': '0'},
              'feature_mappings': {'rw_txn_count_1h': {'schema': os.environ['RISINGWAVE_SCHEMA'], 'view': 'user_features_mv',
                                  'key_column': 'user_id', 'value_column': 'txn_count_1h', 'key_type': 'text'}}}
sources = {name: source}
...
if kind in ('redis', 'risingwave'): (repo / 'lists/db_lists.yaml').unlink()
elif kind == 'postgres':
...
for ds_name, ds in sources.items():
    ...
    if ds['provider'] == 'redis': runtime['namespace'] = 'e2e_features'
...
run_error_test_case() {
    ...
    case "$expected" in
        ...
        feature_error)
            if [ "$HTTP_STATUS" = 500 ] && jq -e '.error.code == "INTERNAL_ERROR"' "$RESPONSE_FILE" >/dev/null 2>&1; then passed=true; fi ;;
    esac
...
if [ "$DATASOURCE" = risingwave ]; then
    source tests/scripts/e2e_test_cases_risingwave.sh
elif [ "$DATASOURCE" = redis ]; then
