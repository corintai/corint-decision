...
///
/// ```rust,ignore
/// use corint_decision_engine::{DecisionEngineBuilder, RepositoryConfig};
///
...
pub struct DecisionEngineBuilder {
    ...
    #[cfg(feature = "sqlx")]
    result_writer: Option<Arc<corint_decision_runtime::DecisionResultWriter>>,
    #[cfg(feature = "sqlx")]
...
impl DecisionEngineBuilder {
    ...
    pub fn with_server_datasources(
        mut self,
        datasources: std::collections::HashMap<
            String,
            corint_decision_runtime::datasource::config::DataSourceConfig,
        >,
    ) -> Self {
    ...
    pub fn with_result_writer(mut self, pool: sqlx::PgPool) -> Self {
        use corint_decision_runtime::DecisionResultWriter;
    ...
    pub async fn build(mut self) -> Result<DecisionEngine> {
        ...
        if self.feature_executor.is_none() {
            if let Some(ref content) = self.repository_content {
                if let Some(executor) = Self::init_feature_executor_from_content(
                    content,
                    &self.repository_config,
                    &self.server_datasources,
                )
                .await?
                {
        ...
        if self.list_service.is_none() {
            if let Some(ref content) = self.repository_content {
                ...
                if let Some(service) =
                    Self::init_list_service_from_content(content, &self.repository_config, db_url)
                        .await?
                {
    ...
    async fn init_feature_executor_from_content(
        ...
        use corint_decision_runtime::datasource::{
            DataSourceClient, DataSourceConfig as RuntimeDataSourceConfig,
        };
        use corint_decision_runtime::feature::registry::FeatureRegistry;
                    ...
                    .map_err(|e| EngineError::Config(e.to_string()))?;
                {
                    ...
                    if let Some(ref server_datasources) = server_datasources {
                        ...
                        for (name, config) in server_datasources.iter() {
                            use corint_decision_runtime::datasource::config::DataSourceType;
                            ...
                            let is_lookup_ds =
                                matches!(&config.source_type, DataSourceType::FeatureStore(_));
                    ...
                    if datasource_count == 0 {
                        let datasource_dir =
                            std::path::Path::new(base_path).join("configs/datasources");
                        if datasource_dir.exists() {
                            tracing::info!(
                                "Loading datasources from repository/configs/datasources/..."
                            );
                                ...
                                for entry in entries.flatten() {
                                    ...
                                    if path.extension().and_then(|s| s.to_str()) == Some("yaml") {
                                        if let Ok(file_content) = std::fs::read_to_string(&path) {
                                            if let Ok(config) =
                                                serde_yaml::from_str::<RuntimeDataSourceConfig>(
                                                    &file_content,
                                                )
                                            {
                                                match DataSourceClient::new(config.clone()).await {
    ...
    async fn init_list_service_from_content(
        ...
        repo_config: &Option<RepositoryConfig>,
        database_url: &Option<String>,
    ) -> Result<Option<corint_decision_runtime::lists::ListService>> {
        use corint_decision_runtime::lists::ListLoader;
        ...
        if let Some(base_path) = base_path {
            ...
            use corint_decision_runtime::datasource::config::{
                DataSourceConfig as RuntimeDataSourceConfig, DataSourceType,
            };
            use corint_decision_runtime::lists::loader::DatasourceInfo;
                ...
                if let Ok(entries) = std::fs::read_dir(&datasource_dir) {
                    for entry in entries.flatten() {
                        ...
                        if path.extension().and_then(|s| s.to_str()) == Some("yaml") {
                            if let Ok(file_content) = std::fs::read_to_string(&path) {
                                if let Ok(config) =
                                    serde_yaml::from_str::<RuntimeDataSourceConfig>(&file_content)
                                {
                                    ...
                                    let (provider, connection_string) = match &config.source_type {
                                        DataSourceType::SQL(sql_config) => {
                                            let provider = match sql_config.provider {
                                                corint_decision_runtime::datasource::config::SQLProvider::SQLite => "sqlite",
                                                corint_decision_runtime::datasource::config::SQLProvider::PostgreSQL => "postgresql",
                                                corint_decision_runtime::datasource::config::SQLProvider::MySQL => "mysql",
                                            };
                                            (
                                                provider.to_string(),
                                                sql_config.connection_string.clone(),
                                            )
                                        }
                                        DataSourceType::OLAP(olap_config) => {
                                            let provider = match olap_config.provider {
                                                corint_decision_runtime::datasource::config::OLAPProvider::ClickHouse => "clickhouse",
                                                corint_decision_runtime::datasource::config::OLAPProvider::Druid => "druid",
                                                corint_decision_runtime::datasource::config::OLAPProvider::TimescaleDB => "timescaledb",
                                                corint_decision_runtime::datasource::config::OLAPProvider::InfluxDB => "influxdb",
                                            };
                                            (
                                                provider.to_string(),
                                                olap_config.connection_string.clone(),
                                            )
                                        }
                                        DataSourceType::FeatureStore(fs_config) => {
                                            ...
                                            (
                                                provider.to_string(),
                                                fs_config.connection_string.clone(),
                                            )
                                        }
                                    };
            ...
            if !datasources.is_empty() {
                ...
                tracing::debug!(
                    "✓ Configured {} datasource(s) for list backends",
                    datasources_count
                );
            }
            ...
            #[cfg(feature = "sqlx")]
            {
                if let Some(db_url) = database_url {
                    ...
                    if db_url.starts_with("sqlite://") || db_url.starts_with("sqlite:") {
                        ...
                        tracing::info!(
                            "✓ SQLite database path configured for list backends: {}",
                            db_path
                        );
                    } else {
                            ...
                            .await
                        {
                            Ok(pool) => {
                                ...
                                tracing::info!(
                                    "✓ Database connection established for list backends"
                                );
                            }
                            Err(e) => {
