...
//!
//! ```rust,ignore
//! use corint_decision_engine::{DslValidator, DslType, ValidationResult};
//!
//! // Validate a rule
//! let rule_yaml = r#"
//! rule:
...
// Re-export all types from corint_decision_compiler::validator
pub use corint_decision_compiler::{
...
"#;
        ...
        assert!(
            result.valid,
            "Expected valid rule, got errors: {:?}",
            result.errors
        );
        assert!(result.errors.is_empty());
        assert_eq!(result.metadata.as_ref().unwrap().doc_type, DslType::Rule);
        assert_eq!(
            result.metadata.as_ref().unwrap().id.as_deref(),
            Some("test_rule")
        );
    }
...
"#;
        ...
        assert!(
            result.valid,
            "Expected valid rule with auto-detection, got errors: {:?}",
            result.errors
        );
        assert_eq!(result.metadata.as_ref().unwrap().doc_type, DslType::Rule);
    }
...
"#;
        ...
        assert!(
            result.valid,
            "Expected valid ruleset, got errors: {:?}",
            result.errors
        );
...
"#;
        ...
        assert!(
            result.valid,
            "Expected valid pipeline, got errors: {:?}",
            result.errors
        );
        assert_eq!(
            result.metadata.as_ref().unwrap().doc_type,
            DslType::Pipeline
        );
    }
}
