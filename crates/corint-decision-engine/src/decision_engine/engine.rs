...
pub struct DecisionEngine {
    ...
    pub(crate) repository_config: Option<corint_decision_repository::RepositoryConfig>,

    /// Feature executor (for reload)
    pub(crate) feature_executor: Option<Arc<corint_decision_runtime::feature::FeatureExecutor>>,

    /// List service (for reload)
    pub(crate) list_service: Option<Arc<corint_decision_runtime::lists::ListService>>,
}
...
impl DecisionEngine {
    ...
    pub async fn new_with_feature_executor(
        ...
        for rule_file in &config.rule_files {
            programs
                .extend(CompilerHelper::load_and_compile_rules(rule_file, &mut compiler).await?);
        }

        // Compile rule contents (from repository)
        for (id, content) in &config.rule_contents {
            programs.extend(
                CompilerHelper::compile_rules_from_content(id, content, &mut compiler).await?,
            );
        }
    ...
    pub async fn decide(&self, mut request: DecisionRequest) -> Result<DecisionResponse> {
        ...
        let mut rule_executions: Vec<corint_decision_runtime::RuleExecutionRecord> = Vec::new();
        ...
        if let Some(ref registry) = self.registry {
                ...
                if matches {
                    ...
                    if let Some(pipeline_program) = self.pipeline_map.get(&entry.pipeline) {
                        ...
                        if !rulesets_to_execute.is_empty() {
                            ...
                            for ruleset_id in &rulesets_to_execute {
                                if let Some(ruleset_program) = self.ruleset_map.get(ruleset_id) {
                                        ...
                                        ruleset_program.metadata.custom.get("rules")
                                    {
                                        ...
                                        for rule_id in rule_ids {
                                            if let Some(rule_program) = self.rule_map.get(rule_id) {
                                                ...
                                                let rule_name =
                                                    rule_program.metadata.name.as_deref();
                                    ...
                                    if let Some(conclusion_json) =
                                        ruleset_program.metadata.custom.get("conclusion_json")
                                    {
                                        result_map.insert(
                            ...
                            .filter(|_| self.core_input_schema.is_none())
                        {
                            ...
                            let decision_program = corint_decision_model::ir::Program {
            ...
            if !pipeline_handled_rules && !pipeline_matched {
                ...
                for program in &rule_programs {
                    ...
                    let conditions_json = program.metadata.custom.get("conditions_json").cloned();
        ...
        if let Some(ref result_writer) = self.result_writer {
            ...
            let decision_record = corint_decision_runtime::DecisionRecord::from_decision_result(
                request_id.clone(),
                event_id,
                pipeline_id,
                &combined_result,
        ...
        let trace = if request.options.enable_trace {
            ...
            let mut rulesets_map: std::collections::HashMap<
                String,
                Vec<&corint_decision_runtime::RuleExecutionRecord>,
            > = std::collections::HashMap::new();
            for rule_exec in &rule_executions {
                let ruleset_id = rule_exec
                    .ruleset_id
                    .clone()
            ...
            if let Some(ref features) = request.features {
                tracing::debug!(
                    "Merging {} pre-provided features into trace_data",
                    features.len()
                );
            ...
            for (ruleset_id, rules) in rulesets_map {
                ...
                {
                    let signal_str = result_map.get("signal").and_then(|v| {
                        if let Value::String(s) = v {
                            Some(s.as_str())
                        } else {
                            None
    ...
    pub async fn reload(&mut self) -> Result<()> {
        ...
        let repo_config = self.repository_config.as_ref().ok_or_else(|| {
            EngineError::Config("Repository not configured. Cannot reload.".to_string())
        })?;
        ...
        let content = loader
            .load_all()
            .await
            .map_err(|e| EngineError::Config(format!("Failed to load repository: {}", e)))?;
        ...
        for (id, content_str) in &new_config.rule_contents {
            programs.extend(
                CompilerHelper::compile_rules_from_content(id, content_str, &mut compiler).await?,
            );
        }
