...
use corint_decision_model::ast::{Expression, WhenBlock};
use corint_decision_model::Value;
use corint_decision_runtime::{ConclusionTrace, ConditionTrace, StepTrace};
...
impl TraceBuilder {
                ...
                let result = match op {
                    LogicalGroupOp::Any => {
                        ...
                        for cond in conditions {
                            let (r, t) =
                                TraceBuilder::evaluate_expression_with_trace(cond, event_data);
    ...
    pub(super) fn evaluate_when_block_with_trace(
        ...
        if let Some(ref conditions) = when.conditions {
            for condition in conditions {
                let (result, trace) =
                    TraceBuilder::evaluate_expression_with_trace(condition, event_data);
                traces.push(trace);
                if !result {
                    return (false, traces);
                }
        ...
        if let Some(binary) = expr.get("Binary") {
            if let Some(obj) = binary.as_object() {
                ...
                let left_val = left_expr
                    .and_then(|e| TraceBuilder::extract_value_from_expr_json(e, event_data));
    ...
    pub(super) fn extract_value_from_expr_json(
        ...
        if let Some(field_access) = expr.get("FieldAccess") {
            if let Some(arr) = field_access.as_array() {
                ...
                let effective_path: Vec<String> = if let Some(first) = path.first() {
                    match first.as_str() {
                ...
                if result.is_none() {
                    tracing::debug!(
                        "Failed to get field value for path {:?}, event_data keys: {:?}",
                        effective_path,
                        event_data.keys().collect::<Vec<_>>()
                    );
                }
                return result;
            }
    ...
    pub(super) fn json_value_to_condition_trace(
        ...
        match expr_type {
            "group" => {
                ...
                let nested_conditions: Vec<ConditionTrace> = json
                    ...
                    .map(|arr| {
                        arr.iter()
                            .map(|c| {
                                TraceBuilder::json_value_to_condition_trace(c, false, event_data)
                            })
                            .collect()
                    })
                    .unwrap_or_default();
            ...
            "binary" => {
                ...
                let left_value_display = if is_boolean_literal { None } else { left_val };
            ...
            _ => {
                ...
                let result = if let Some(val) =
                    TraceBuilder::extract_value_from_json_expr(json, event_data)
                {
    ...
    pub(super) fn extract_value_from_json_expr(
        ...
        match expr_type {
            ...
            "literal" => {
                // Literal value - convert from serde_json::Value to corint_decision_model::Value
        ...
        match json {
            ...
            serde_json::Value::Array(arr) => {
                let values: Vec<Value> = arr
                    .iter()
                    .filter_map(TraceBuilder::json_to_core_value)
                    .collect();
            ...
            serde_json::Value::Object(obj) => {
                let map: HashMap<String, Value> = obj
                    .iter()
                    .filter_map(|(k, v)| {
                        TraceBuilder::json_to_core_value(v).map(|val| (k.clone(), val))
                    })
                    .collect();
