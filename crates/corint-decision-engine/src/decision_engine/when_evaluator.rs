...
impl WhenEvaluator {
        ...
        match group {
            ConditionGroup::All(conditions) => {
                ...
                for condition in conditions {
                    let (result, mut cond_traces) =
                        WhenEvaluator::evaluate_condition_with_trace(condition, event_data);
    ...
    pub(super) fn evaluate_binary_expression(
        ...
        match op {
            ...
            Operator::And => {
                WhenEvaluator::is_truthy(&left_val) && WhenEvaluator::is_truthy(&right_val)
            }
            Operator::Or => {
                WhenEvaluator::is_truthy(&left_val) || WhenEvaluator::is_truthy(&right_val)
            }
