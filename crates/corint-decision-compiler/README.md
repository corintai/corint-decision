...
`corint-decision-compiler` compiles Abstract Syntax Tree (AST) representations of decision rules into executable Intermediate Representation (IR). It handles semantic analysis, type checking, and optimization before generating runtime-executable code.
...
```
┌─────────────────────────────────────┐
│   AST (from corint-decision-dsl-parser)         │
...
└─────────────┬───────────────────────┘
              ...
              ▼
┌─────────────────────────────────────┐
│   IR (for corint-decision-runtime)          │
...
```rust
use corint_decision_compiler::Compiler;
use corint_decision_repository::FileSystemRepository;
...
- `corint-decision-model`: AST and IR type definitions
- `corint-decision-dsl-parser`: YAML to AST parsing
- `corint-decision-repository` (optional): For import resolution
