...
use corint_decision_model::ast::{Expression, LogicalGroupOp, Operator};
        ...
        match expr {
            Expression::Literal(val) => {
                let val_json = match val {
                    corint_decision_model::Value::String(s) => json!(s),
    ...
    pub fn expression_to_string(expr: &Expression) -> String {
        match expr {
            Expression::Literal(val) => match val {
                corint_decision_model::Value::String(s) => format!("\"{}\"", s),
                corint_decision_model::Value::Number(n) => {
                    if n.fract() == 0.0 {
                        format!("{}", *n as i64)
                    } else {
                        n.to_string()
                ...
                corint_decision_model::Value::Bool(b) => b.to_string(),
                corint_decision_model::Value::Null => "null".to_string(),
                corint_decision_model::Value::Array(arr) => format!(
                    "[{}]",
                    arr.iter()
                        .map(|v| match v {
                            corint_decision_model::Value::String(s) => format!("\"{}\"", s),
                            corint_decision_model::Value::Number(n) => n.to_string(),
                            _ => format!("{:?}", v),
                        })
                        .collect::<Vec<_>>()
                        .join(", ")
                ),
                corint_decision_model::Value::Object(_) => "{...}".to_string(),
            },
