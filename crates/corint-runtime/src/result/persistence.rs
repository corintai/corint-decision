...
impl DecisionResultWriter {
    ...
    pub fn write_decision(&self, record: DecisionRecord) -> Result<()> {
        self.sender.send(record)
            .map_err(|e| RuntimeError::RuntimeError(format!("Failed to queue decision record: {}", e)))
    }
    ...
    async fn process_records(
        ...
        while let Some(record) = receiver.recv().await {
            tracing::debug!("Processing decision record for request_id: {}", record.request_id);
            
            match Self::write_to_database(&pool, &record).await {
                Ok(()) => {
                    tracing::info!("Successfully persisted decision record for request_id: {}", record.request_id);
                }
                Err(e) => {
                    tracing::error!("Failed to write decision record to database for request_id {}: {}", record.request_id, e);
    ...
    async fn write_to_database(
        ...
        tracing::debug!("Starting database transaction for request_id: {}", record.request_id);
        ...
        let triggered_rules_array: Vec<&str> = record.triggered_rules.iter().map(|s| s.as_str()).collect();
        
        // Convert rule_scores to JSONB
        let rule_scores_json = serde_json::to_value(&record.rule_scores)
            .map_err(|e| RuntimeError::RuntimeError(format!("Failed to serialize rule_scores: {}", e)))?;
        
        // Convert feature_values to JSONB
        let feature_values_json = if let Some(ref fv) = record.feature_values {
            Some(serde_json::to_value(fv)
                .map_err(|e| RuntimeError::RuntimeError(format!("Failed to serialize feature_values: {}", e)))?)
        } else {
            None
        };
        ...
        match insert_result {
            ...
            Err(e) => {
                ...
                return Err(RuntimeError::RuntimeError(format!("Failed to insert risk_decision: {}", e)));
            }
        }
        ...
        for rule_exec in &record.rule_executions {
            let feature_values_json = if let Some(ref fv) = rule_exec.feature_values {
                Some(serde_json::to_value(fv)
                    .map_err(|e| RuntimeError::RuntimeError(format!("Failed to serialize rule feature_values: {}", e)))?)
            } else {
                None
            };
            ...
            match exec_result {
                ...
                Err(e) => {
                    ...
                    return Err(RuntimeError::RuntimeError(format!("Failed to insert rule_execution: {}", e)));
                }
            }
        }
        
        tracing::info!("Inserted {} rule_execution records for request_id: {}", 
               record.rule_executions.len(), record.request_id);
        
        // Commit transaction
        let commit_result = tx.commit().await;
        match commit_result {
            Ok(()) => {
                tracing::info!("Transaction committed successfully for request_id: {}", record.request_id);
            }
            Err(e) => {
                tracing::error!("Failed to commit transaction for request_id {}: {}", record.request_id, e);
                return Err(RuntimeError::RuntimeError(format!("Failed to commit transaction: {}", e)));
            }
        }
...
impl DecisionRecord {
    ...
    pub fn from_decision_result(
        ...
        let feature_values = result.context.get("feature_values")
            .and_then(|v| {
