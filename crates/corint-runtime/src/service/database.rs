...
mod tests {
    ...
    async fn test_database_service_client() {
        ...
        let request = ServiceRequest::new("database".to_string(), "query".to_string()).with_param(
            "query".to_string(),
            Value::String("SELECT COUNT(*) FROM transactions".to_string()),
        );
    ...
    async fn test_database_multiple_params() {
        let query =
            DatabaseQuery::new("SELECT * FROM users WHERE name = ? AND age > ?".to_string())
    ...
    async fn test_database_service_execute() {
        ...
        let request = ServiceRequest::new("database".to_string(), "execute".to_string())
            .with_param(
                "query".to_string(),
                Value::String("DELETE FROM users WHERE id = 1".to_string()),
            );
    ...
    async fn test_database_seed_multiple_tables() {
        ...
        // Query both tables
        let users = client
            .query(DatabaseQuery::new("SELECT * FROM users".to_string()))
            .await
            .unwrap();
        let orders = client
            .query(DatabaseQuery::new("SELECT * FROM orders".to_string()))
            .await
            .unwrap();
