...
pub async fn load_rules_from_repository(
    ...
    let repo: Box<dyn Repository> = match repository {
        ...
        RepositoryType::Database { db_type, url } => {
            info!("Loading rules from database repository: {:?} at {}", db_type, url);
            ...
            match db_type {
                DatabaseType::PostgreSQL => {
                    ...
                    #[cfg(not(feature = "postgres"))]
                    {
                        ...
                        return Err(anyhow::anyhow!("Enable 'postgres' feature to use PostgreSQL repository"));
                    }
                }
                DatabaseType::MySQL => {
        ...
        RepositoryType::Api { base_url, api_key } => {
            ...
            Box::new(ApiRepository::new(base_url, api_key.as_deref())
                .await
    ...
    let pipeline_ids = repo.list_pipelines().await.map_err(|e| {
        anyhow::anyhow!("Failed to list pipelines: {}", e)
    })?;
    ...
    for pipeline_id in pipeline_ids {
        ...
        let (_, content) = repo.load_pipeline(&pipeline_id).await.map_err(|e| {
            anyhow::anyhow!("Failed to load pipeline {}: {}", pipeline_id, e)
        })?;
    ...
    info!("✓ Successfully loaded {} pipelines from repository", pipeline_count);
