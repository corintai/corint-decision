//! Basic usage example for corint-decision-model
//!
//! Run with: cargo run --example basic_usage

use corint_decision_model::ast::{Expression, Operator, Rule, WhenBlock};
...
fn main() {
    ...
    println!(
        ...
        rule.when.conditions.as_ref().map(|c| c.len()).unwrap_or(0)
    );
    println!("\n   Full Rule: {:#?}\n", rule);
