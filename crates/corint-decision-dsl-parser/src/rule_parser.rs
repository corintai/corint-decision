...
use corint_decision_model::ast::{CdlDocument, Expression, Rule, WhenBlock};
...
impl RuleParser {
    ...
    pub fn parse_with_imports(yaml_str: &str) -> Result<CdlDocument<Rule>> {
        ...
        if let Some(imports) = imports {
            Ok(CdlDocument::with_imports(version, imports, rule))
        } else {
            Ok(CdlDocument::new(version, rule))
        }
    }
    ...
    fn parse_condition(yaml: &YamlValue) -> Result<Expression> {
        if let Some(s) = yaml.as_str() {
            ...
            Err(ParseError::InvalidValue {
                ...
                message: "Condition must be a string expression or logical group (any/all)"
                    .to_string(),
            })
        }
    }
            ...
            seq.iter()
                .map(|item| {
                    if let Some(s) = item.as_str() {
                        ...
                        Err(ParseError::InvalidValue {
                            field: "condition".to_string(),
                            message: "Each condition must be a string or nested logical group"
                                .to_string(),
                        })
                    }
                })
    ...
    /// Parse params object
    fn parse_params(params_obj: &YamlValue) -> Result<corint_decision_model::ast::RuleParams> {
        use corint_decision_model::ast::RuleParams;
