...
`corint-decision-runtime` executes compiled decision logic (IR) in production environments. It provides the core execution engine, feature calculation, datasource integration, and runtime context management.
...
```rust
use corint_decision_runtime::FeatureExecutor;
...
```rust
use corint_decision_runtime::datasource::{PostgresClient, RedisClient, DatasourceRegistry};
