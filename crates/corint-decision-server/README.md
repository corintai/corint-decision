...
- ✅ serde_json::Value to corint_decision_model::Value conversion
...
#### 1. Integration with corint-decision-sdk
...
- `corint-decision-sdk` - DecisionEngine API
- `corint-decision-runtime` - Runtime execution engine
- `corint-decision-model` - Core type definitions
...
```bash
# Run unit tests
cargo test --package corint-decision-server

# Run with test coverage
cargo tarpaulin --package corint-decision-server
```
...
```bash
# Debug level
RUST_LOG=debug cargo run -p corint-decision-server

# Module-level logging
RUST_LOG=corint_decision_server=debug,corint_decision_runtime=info cargo run -p corint-decision-server
```
...
```bash
# Custom port
CORINT_PORT=9090 cargo run -p corint-decision-server
```
...
WORKDIR /app
COPY . .
RUN cargo build --release -p corint-decision-server

FROM debian:bookworm-slim
COPY --from=builder /app/target/release/corint-decision-server /usr/local/bin/
EXPOSE 8080
CMD ["corint-decision-server"]
```
...
spec:
  ...
  template:
    spec:
      containers:
      - name: corint-decision-server
        image: corint-decision-server:latest
        ports:
        - containerPort: 8080
```
