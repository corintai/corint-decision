...
```bash
cargo build -p corint-decision-ffi --release
```

Outputs:
- macOS: `target/release/libcorint_ffi.dylib`
- Linux: `target/release/libcorint_ffi.so`
- Windows: `target/release/corint_decision_ffi.dll`
...
```bash
python3 crates/corint-decision-ffi/bindings/python/example.py
```

### Go

```bash
cd crates/corint-decision-ffi/bindings/go
...
```bash
cd crates/corint-decision-ffi/bindings/typescript
npm install
npm run build
npm run example
npm run example:ts
...
```bash
cd crates/corint-decision-ffi/bindings/java
...
- **Library not found**: ensure `cargo build -p corint-decision-ffi --release` has been run.
