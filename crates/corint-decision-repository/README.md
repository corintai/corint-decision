...
```toml
[dependencies]
corint-decision-repository = { path = "crates/corint-decision-repository" }

# For database support
corint-decision-repository = { path = "crates/corint-decision-repository", features = ["postgres"] }
```
...
```rust
use corint_decision_repository::{Repository, FileSystemRepository};
...
```rust
use corint_decision_repository::{Repository, PostgresRepository, WritableRepository};
...
```rust
use corint_decision_repository::{FileSystemRepository, CacheConfig};
...
```rust
use corint_decision_repository::CacheableRepository;
...
```rust
use corint_decision_compiler::{Compiler, ImportResolver};
use corint_decision_repository::{Repository, FileSystemRepository, PostgresRepository};
use std::sync::Arc;
...
```bash
cargo test --package corint-decision-repository
```

Run specific test:

```bash
cargo test --package corint-decision-repository test_file_system_load_rule
```
...
cargo test --package corint-decision-repository --features postgres -- --ignored

# Run specific database test
cargo test --package corint-decision-repository --features postgres test_postgres_save_rule -- --ignored
```
...
cargo test --package corint-decision-compiler integration_test_with_repository

# Test with PostgreSQL repository
export DATABASE_URL="postgresql://localhost/corint_test"
cargo test --package corint-decision-compiler --features postgres integration_test_with_database -- --ignored
```
...
```rust
use corint_decision_repository::RepositoryError;
...
cargo bench --package corint-decision-repository -- file_system

# Benchmark PostgreSQL repository
export DATABASE_URL="postgresql://localhost/corint_test"
cargo bench --package corint-decision-repository --features postgres -- postgres
```
...
```rust
env_logger::builder()
    .filter_module("corint_decision_repository", log::LevelFilter::Debug)
    .init();
```
...
```bash
export RUST_LOG=corint_decision_repository=debug
cargo run
```
