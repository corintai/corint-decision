...
先从 `tests/conformance/generation/` 读取四种资源模板，以及 `tests/conformance/cdl_core/` 的输入和行为 fixture，再写入策略目录。
...
以仓库 [testing.md](../../../docs/cdl/testing.md) 和 [test-suite.json](../../../docs/cdl/schema/test-suite.json) 为准：
...
- 测试输入的 `event` 包装与 `record --event` 所读的裸事件对象不同。只有需要回放时才读取[回放契约](../../../docs/cdl/replay.md)。
...
- **绑定证据的源码包**：读取[包契约](../../../docs/cdl/packages.md)，使用 `cdl build --input-schema "$POLICY_DIR/input-schema.yaml" --cases "$POLICY_DIR/behavior.yaml" --output "$POLICY_DIR/policy.core-package.json" --format json "$@"`。输出路径必须不存在；`build` 会重新测试，`verify --package ... --cases ...` 检查绑定并重新执行用例。
- **模块化创作目录**：读取[解析契约](../../../docs/cdl/resolution.md)。`resolve` 只解析、冻结并编译；随后 `import --bundle ... --cases ... --output ...` 重新测试并生成源码包。冻结 bundle 是 JSON 容器，不能把它当作 CDL 资源文件传给 `validate`。
...
- **发布或服务激活**：这是独立工作流，读取[Agent 交付契约](../../../docs/cdl/agent-workflow.md)并遵守已有授权。编写任务不自动启动服务或操作部署。
